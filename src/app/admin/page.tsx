...
export default function AdminPage() {
  const [session, setSession] = useState<{ role: string; key: string } | null>(
    null
  );
...
function AdminLogin({
  ...
  onLogin: (s: { role: string; key: string }) => void;
}) {
          ...
          {canManageTech && (
            <PointsAssignmentPanel
              ...
              members={members.filter(
                (m) => m.role === 'Technical Contributor'
              )}
...
function PointsAssignmentPanel({
  title,
  members,
  onRefresh,
}: {
    ...
    <div className="bg-surface-dark border border-white/10 rounded-2xl p-6">
      ...
      >
        ...
        <button
          type="submit"
          disabled={adding}
          className="bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg flex items-center justify-center gap-2 py-2"
        >
          <Plus size={16} /> Add
        </button>
      </form>
          ...
          <tbody>
            {members.map((m) => (
              ...
              >
                ...
                <td className="p-3">
                  <span
                    className={`px-2 py-1 rounded text-xs ${m.role === 'City Lead' ? 'bg-sky-500/20 text-sky-300' : 'bg-emerald-500/20 text-emerald-300'}`}
                  >
                    {m.role}
                  </span>
                </td>
                ...
                <td className="p-3 text-right">
                  <button
                    ...
                    className="p-2 hover:bg-rose-500/20 text-rose-400 rounded transition-colors"
                  >
                    <Trash2 size={16} />
                  </button>
                </td>
